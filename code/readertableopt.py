...
def get_opt_table_by_question(inp_question_id, answer_table_uid, topn, df_tables_opt):
    position = f'top{topn+1}'
    scores = df_tables_opt[(df_tables_opt['question_id'] == inp_question_id) & (df_tables_opt['position'] == position)]['column_scores']
    cols = eval(df_tables_opt[(df_tables_opt['question_id'] == inp_question_id) & (df_tables_opt['position'] == position)].iloc[0,7])
    mismatch = df_tables_opt[(df_tables_opt['question_id'] == inp_question_id) & (df_tables_opt['position'] == position)].iloc[0,11]
    if 'metadata' in cols:  # falha no llm
        return ('error','error')

    if mismatch == "mismatch":
        return ('error','error')

 #   binary_scores = [0 if score == 0 else 1 for score in scores]
    #scores = eval(scores[0])
    scores = eval(scores.iloc[0])
    binary_scores = []
    for score in scores:
        if score <= 0.7:   #nao uso a otimizacao
            binary_scores.append(0)
        else:              #uso a otimizacao
            binary_scores.append(1)
    

    # Verificar se há pelo menos um acima do score
    if binary_scores.count(1) < 1:
        return ('error','error')
    table = pd.read_csv(answer_table_uid, sep=',')
    # Remover colunas com base em binary_scores

    binary_scores[0] = 1  # forcando a primeira coluna sempre estar presente
    columns_to_keep = [col for col, score in zip(table.columns, binary_scores) if score != 0]
    # Filtrar o DataFrame para manter apenas as colunas desejadas
    return (table[columns_to_keep],'ok')
...
def build_reader(retriever_file, device, run_improve_question):
            ...
            #dataset = dataset.drop(dataset.index[10:])  ## apenas 10 

            df = pd.DataFrame()
            count_EM_tapex = 0
            count_EM_tapas = 0
#aqui verificar 
            df_tables_opt = pd.read_csv('/data/tat-qa/llm_table_opt/fromrerankingcenario4/mpnet_table_intro_embeddings_cpu_512_512_filtered.full.csv', sep=',') 
            #df_tables_opt = pd.read_csv('/data/tat-qa/llm_table_opt/filter002/mpnet_table_intro_embeddings_cpu_512_512_filtered.full.csv', sep=',') 
...
#tenho que pegar a saida do rr misturando com a saida da otimizacao
...
#                else:    
                ...
                inp_question_id   = data['question_id']
                ...
                status_table_opt_list = []

                print()
                print(f'linha {index}')
                print()
                ## atencao
                for topn, answer_table_uid in enumerate(answer_table_uid_list[0:10]):

                    table = answer_table_uid = answer_table_uid.replace("'", "").replace('"', '').replace('(', '').replace(')', '').replace(',', '.').replace('*', '')  
                    #AQUI
                    table_opt,status = get_opt_table_by_question(inp_question_id, answer_table_uid, topn, df_tables_opt)
                    #optimized = True

                    ### status = 'error'   ## FORCANDO BASELINE

                    if status == 'error':  # tabela original
                        #table = "/data/ott-qa/new_csv/" + answer_table_uid + ".csv"  ==> ott-qa
                        table = pd.read_csv(table, sep=',', index_col=None)
                        table = table.astype(str) # necessário para o tapex
                        optimized = False
                    else:
                        #table_opt = pd.read_csv(table, sep=',', index_col=None)
                        optimized = True
                        table = table_opt.astype(str) # necessário para o tapex
                        print('otimizou!!!!!')
...
#                    result = get_tapas_result(inp_question,results,aggregation_predictions_string)
                    ...
                    status_table_opt_list.append(optimized)
                    print("")
                ...
                new_data = {'tapex_answer_text'   : tapex_answer_text_list, #,
                            ...
                            'status_table_opt'    : status_table_opt_list}
...
def concatena_files():
    ...
    df_file1 = df_file1.drop(df_file1.index[250:])  ## teste
    print(df_file1.shape)
    ...
    df_reader = pd.concat([df_file1.reset_index(drop=True), df_file3.reset_index(drop=True)], axis=0)
